...
class MoveJPDemo(Node):
    def __init__(self):
        ...
        self.movej_way = Movej()
        self.movej_way.joint = [0.0]*6
        
        #创建发布者和订阅器
        self.test_publisher = self.create_publisher(Movej,"/rm_driver/movej_cmd",10)
        self.test_subscriber = self.create_subscription(Bool,"/rm_driver/movej_result",self.movej_callback,10)
        ...
        self.d1,self.d2,self.d3,self.d4,self.d5,self.d6,self.a2 = 0.1020,0.2560,0.1285,0.0815,0.115,0.080,0.256
    ...
    def execute_next_move(self):
        ...
        try:
            if not self.parameters_queue.empty():
                ...
                self.movej_demo(parameters)
        except queue.Empty:
            pass
        except Exception as e:
            self.get_logger().error(f"执行命令错误:{str(e)}")   
            
    def movej_demo(self,parameters):
        """movej运动规划函数"""
        #设置关节角度
        self.movej_way.joint[0] = parameters[0]
        self.movej_way.joint[1] = parameters[1]
        self.movej_way.joint[2] = parameters[2]
        self.movej_way.joint[3] = parameters[3]
        self.movej_way.joint[4] = parameters[4]
        self.movej_way.joint[5] = parameters[5]
        
        #设置其他参数
        self.movej_way.speed = 20
        self.movej_way.trajectory_connect = 0
        self.movej_way.block = True
        #发布消息
        self.test_publisher.publish(self.movej_way)
         ...
         while rclpy.ok():
            if select.select([sys.stdin],[],[],0.1)[0]:
                ...
                if not all(s.replace(".","",1).replace('-','',1).isdigit() for s in position_input):
                    ...
                    continue    
                if len(position_input) != 6:
                    test_move.get_logger().info("请输入六个数字")
                    continue
