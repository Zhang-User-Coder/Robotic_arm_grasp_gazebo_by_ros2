...
void MoveJPDemo::MoveJPDemo_Callback(const std_msgs::msg::Bool & msg)
{
    ...
    if(msg.data)
    {
        RCLCPP_INFO (this->get_logger(),"*******MoveBK succeeded\n");
    } else {
        RCLCPP_INFO (this->get_logger(),"*******MoveBK Failed\n");
    }
}   
/***********************************************end**************************************************/
...
void MoveJPDemo::movejp_demo()
{
    ...
    moveJ_P_TargetPose.pose.position.x = 0;
    moveJ_P_TargetPose.pose.position.y = 0;
    moveJ_P_TargetPose.pose.position.z = 0.85;
    moveJ_P_TargetPose.pose.orientation.x = 0;
    moveJ_P_TargetPose.pose.orientation.y = 0;
    moveJ_P_TargetPose.pose.orientation.z = 0;
